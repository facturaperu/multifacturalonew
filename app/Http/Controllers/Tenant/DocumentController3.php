...
class DocumentController3 extends Controller
{
    use StorageDocument;
    ...
    {
        $records = Document::with(['soap_type', 'state_type', 'customer', 'note', 'invoice'])
                            ->latest();
        if($request->search) {
            $records = $records->where('number',$request->search);
        }
        $records = $records->paginate(env('ITEMS_PER_PAGE',10));
        $response = [
            'pagination' => [
                'total' => $records->total(),
